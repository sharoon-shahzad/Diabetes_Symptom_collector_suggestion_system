...
export default function ManageAdmins() {
  ...
  const loadUsers = async () => {
    setLoading(true);
    try {
      const res = await axiosInstance.get('/users/allAdmins');
          ...
          <>
                      ...
                      >
                        ...
                        <TableCell align="right">
                          <Box display="flex" gap={0.5} justifyContent="flex-end" alignItems="center">
                            <Tooltip title={isSuperAdminUser && !isCurrentUser ? "Cannot edit other Super Admins" : "Edit Admin"} arrow>
                              <span>
                                <IconButton 
                                  size="small"
            ...
            <Box display="flex" justifyContent="space-between" alignItems="center" p={2}>
              <Typography variant="body2" color="text.secondary">
                Showing {startIndex + 1}-{Math.min(endIndex, filteredUsers.length)} of {filteredUsers.length} admins
              </Typography>
              <Pagination
                count={totalPages}
                page={page}
